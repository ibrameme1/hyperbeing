...
- **`suggestPlanForCost` compares monthly allocation to incremental need** — it suggests the first plan whose *total* monthly credits ≥ shortfall, not one whose *remaining* delta covers it; upsell can under-suggest (`config/credits.js:57`). Fix: compare against `creditsNeeded + (currentPlanCredits - creditsRemaining)` or just document the intent.
- **`clearStaleTestSubscriptions` on every boot** calls Stripe serially for every subscription row (`stripeService.js:258`) — slow start at scale; move behind an env flag or batch.
...
- **`design.js` retry ignores the 8-parallel cap** (`/:id/retry` doesn't re-check in-flight count).
