...
**Remediation pass (2026-07-08, same day):** every item now carries a STATUS line. Nearly all are fixed; the notable survivors are linter/CI (#4) and SSE query-param tokens (#8b); #7 (images out of SQLite) was since completed via on-disk storage. Two extra bugs surfaced and were fixed during remediation: `emailService.js` crashed the entire server at boot when `RESEND_API_KEY` was unset (Resend's constructor throws on a missing key — now lazily constructed), and mock-mode generation always produced an empty deck because `generateCompactPlan` called `mockChat([])` with zero user turns. The chat-flow feature (Nova chat inside a presentation + its legacy serial pipeline) was **removed entirely** rather than patched — it was unreachable dead weight with a billing hole.*
...
> **FIXED (2026-07-08)** — all slide images (current, versions, and thumbnails) now live on disk via `services/imageStore.js`, referenced from the DB by an authed URL + `image_file` filename instead of inline base64. A one-time boot migration (`migrateImagesToDisk`, `PRAGMA user_version` 2) moves existing decks over, writing files before rewriting rows so a crash can't lose an image; read paths still serve legacy inline data during the window. Images served by `GET /:id/slides/:index/image` (and `/versions/:vid/image`), cleaned up on slide/deck delete and version prune. Set `IMAGE_STORAGE_DIR` to the persistent-volume path in prod (defaults next to the SQLite file). Covered by `test/imageStore.test.js` + `test/imageMigration.test.js`. NOTE: this is the **local-disk (Option B)** approach — it keeps the app single-instance (see #8/#18 for the other single-process assumptions); moving to object storage later is a one-file change behind `imageStore`.
