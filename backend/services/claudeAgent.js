...
// Exported for unit tests only — not part of the service's public surface.
export function extractPrefixedObjects(text) {
  const objects = [];
  let pos = 0;
...
export async function generateCompactPlan(message, attachments, userId = null, callbacks = {}) {
  ...
  if (MOCK_MODE) {
    ...
    // Pass the brief as a user turn — with an empty history mockChat stays in
    // 'gathering_info' and never returns the demo plan, which made mock-mode
    // generation complete with zero slides.
    const mock = await mockChat([{ role: 'user', content: message }]);
    if (mock.slide_plan) {
