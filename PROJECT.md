...
Since 2026-07, **slide images no longer live in the database at all** — they're written to disk (`services/imageStore.js`, on the persistent volume in prod via `IMAGE_STORAGE_DIR`) and `slides_data[].image_data` holds an authed URL plus an `image_file` filename. Only tiny SVG placeholders stay inline (so failure-detection keeps working). Version images live on disk too, referenced from the `slide_versions` table (capped at 10/slide). A one-time boot migration (`migrateImagesToDisk`) moved existing decks over. This is the local-disk ("Option B") fix for the old base64-in-SQLite bloat: it shrinks rows ~orders of magnitude and stops every edit from rewriting all image bytes, at the cost of keeping the app single-instance (the volume belongs to one machine). Dashboard list payloads carry `has_thumbnail`; images stream from `GET /:id/thumbnail`, `GET /:id/slides/:index/image`, and `GET /:id/slides/:index/versions/:vid/image` (all authed with `?token=`, since `<img>` can't send headers).
